...
-- First, let's add some sample creators
...
-- Add some sample course enrollments and reviews
-- First, let's add some mock user IDs (in a real scenario, these would be actual users)
DO $$
DECLARE
  course_react_id UUID;
  course_design_id UUID;
  course_marketing_id UUID;
  course_python_id UUID;
  course_mobile_id UUID;
  course_social_id UUID;
  mock_user_1 UUID := gen_random_uuid();
  mock_user_2 UUID := gen_random_uuid();
  mock_user_3 UUID := gen_random_uuid();
  mock_user_4 UUID := gen_random_uuid();
  mock_user_5 UUID := gen_random_uuid();
BEGIN
  -- Get course IDs
  SELECT id INTO course_react_id FROM public.courses WHERE title = 'Complete React Developer Bootcamp 2024';
  SELECT id INTO course_design_id FROM public.courses WHERE title = 'UI/UX Design Masterclass: From Beginner to Pro';
  SELECT id INTO course_marketing_id FROM public.courses WHERE title = 'Digital Marketing Blueprint: Scale Any Business';
  SELECT id INTO course_python_id FROM public.courses WHERE title = 'Python for Data Science & Machine Learning';
  SELECT id INTO course_mobile_id FROM public.courses WHERE title = 'Mobile App Design: iOS & Android UI/UX';
  SELECT id INTO course_social_id FROM public.courses WHERE title = 'Social Media Marketing Mastery 2024';

  -- Add course reviews to make them look realistic
  ...
  (course_react_id, mock_user_1, 5, 'Incredible course! Changed my career', 'This course is absolutely amazing. Sarah explains everything so clearly and the projects are practical and engaging. I went from knowing nothing about React to building my own applications. Highly recommend!', true, 23, now() - interval '2 months'),
  (course_react_id, mock_user_2, 5, 'Best React course on the internet', 'I have taken many React courses but this one stands out. The structure is perfect, starting from basics and gradually building complexity. The TypeScript integration is excellent.', true, 18, now() - interval '1 month'),
  (course_react_id, mock_user_3, 4, 'Great content, sometimes moves fast', 'Overall excellent course with high-quality content. Sometimes the pace is a bit quick for complete beginners, but if you pause and practice, it is worth it.', true, 12, now() - interval '3 weeks'),
  
  -- Design course reviews  
  (course_design_id, mock_user_1, 5, 'From zero to design hero!', 'Marcus is an incredible instructor. This course took me from having no design skills to creating professional-looking interfaces. The Figma tutorials are top-notch.', true, 31, now() - interval '6 weeks'),
  (course_design_id, mock_user_4, 5, 'Perfect for career transition', 'I was a developer wanting to learn design. This course gave me all the skills I needed to transition into a UX role. The portfolio section was especially valuable.', true, 19, now() - interval '1 month'),
  (course_design_id, mock_user_5, 4, 'Comprehensive and well-structured', 'Covers everything you need to know about UI/UX design. The real client projects add great practical value. Would recommend to anyone starting in design.', true, 15, now() - interval '2 weeks'),
  
  -- Marketing course reviews
  (course_marketing_id, mock_user_2, 5, 'ROI paid for itself in first week', 'Elena knows what she is talking about. Applied her Facebook ads strategy and got immediate results. This course more than paid for itself in the first week.', true, 42, now() - interval '5 weeks'),
  (course_marketing_id, mock_user_3, 5, 'Game-changing strategies', 'Finally, a marketing course that actually works! The email marketing section alone is worth the price. My business has grown 300% since taking this course.', true, 38, now() - interval '3 weeks'),
  
  -- Python course reviews
  (course_python_id, mock_user_4, 4, 'Great introduction to data science', 'Perfect course for beginners in data science. Sarah breaks down complex concepts into digestible pieces. The machine learning projects are fantastic.', true, 16, now() - interval '1 month'),
  (course_python_id, mock_user_5, 5, 'Landed my first data science job!', 'This course gave me the foundation I needed to break into data science. The portfolio projects helped me stand out in interviews. Highly recommended!', true, 22, now() - interval '2 months'),
  
  -- Mobile design reviews
  (course_mobile_id, mock_user_1, 4, 'Solid mobile design fundamentals', 'Great course for learning mobile design principles. Marcus covers both iOS and Android guidelines thoroughly. The Figma templates are very useful.', true, 14, now() - interval '3 weeks'),
  
  -- Social media reviews
  (course_social_id, mock_user_2, 5, 'My Instagram following exploded!', 'Elena reveals strategies that actually work. My Instagram went from 500 to 50K followers in 3 months using her techniques. This course is gold!', true, 35, now() - interval '1 month');
